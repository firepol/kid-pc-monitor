...
# Read-only "kid" status page: a friendly browser page that shows the kid how
# much time is left. It only reads from PCTimeControl - it cannot change any
# limits, lock the PC, or run commands.
    ...
    class KidStatusHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path.startswith("/api/status"):
                body = json.dumps(pc_control.get_kid_status()).encode("utf-8")
...
class KidStatusServer:
    ...
    def __init__(self, pc_control, port=KID_PAGE_PORT):
        self.pc_control = pc_control
        self.port = port
        self.httpd = None
...
if __name__ == "__main__":
    ...
    if KID_PAGE_PORT:
        kid_server = KidStatusServer(control)
        kid_thread = threading.Thread(target=kid_server.start, daemon=True)
        kid_thread.start()
